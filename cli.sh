...
echo -e "$BLUE│$NC ${ORANGE}•${NC} MiladMk Link: \"http://$domain:$def_port/$def_cp/login\""
...
echo -e " ${BLUE}[4]${NC} ${ORANGE}•${NC} ${WHITE}Update MiladMk Panel${NC}"
echo -e " ${BLUE}[5]${NC} ${ORANGE}•${NC} ${WHITE}Remove Panel${NC}"
echo -e " ${BLUE}[6]${NC} ${ORANGE}•${NC} ${WHITE}Remove All Admin${NC}"
...
;;
4)
bash <(curl -Ls https://github.com/MiladMk/MiladMk-SSH-User-Management/raw/main/install.sh --ipv4)
;;
5)
...
;;
7)
bash <(curl -Ls https://github.com/MiladMk/MiladMk-SSH-User-Management/raw/main/block_iran.sh --ipv4)
;;
8)
sudo iptables -F
;;
9)
if curl -sfL "https://raw.githubusercontent.com/MiladMk/MiladMk-SSH-User-Management/main/dropbear/install.sh" -o /tmp/dropbear-install.sh 2>/dev/null && [ -s /tmp/dropbear-install.sh ]; then
bash /tmp/dropbear-install.sh
else
echo "Dropbear installer not found in your repo (dropbear/install.sh). Skipping."
fi
;;
10)
bash <(curl -Ls https://raw.githubusercontent.com/MiladMk/MiladMk-SSH-User-Management/main/wp-install.sh --ipv4)
;;
11)
bash <(curl -Ls https://raw.githubusercontent.com/MiladMk/MiladMk-SSH-User-Management/main/fix-call.sh --ipv4)
;;
12)
bash <(curl -Ls https://raw.githubusercontent.com/MiladMk/MiladMk-SSH-User-Management/main/OT/singbox.sh --ipv4)
;;
13)

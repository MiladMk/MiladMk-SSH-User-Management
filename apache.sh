...
echo "This Script only supported in ubuntu 20 and above"
exit
fi
fi

# ---- PHP version selection (see install.sh for rationale) ----------------
PHPV="8.3"
ubu_major=$(lsb_release -rs | cut -d '.' -f 1)
if [ "$ubu_major" -le 22 ] 2>/dev/null; then
  PHPV="8.1"
fi
export PHPV
userDirectory="/home"
...
echo -e "${YELLOW}************ Select MiladMk Version Apache Web Server************"
...
sudo apt install python3 python-is-python3 -y 2>/dev/null || sudo apt install python3 -y
if [ ! -e /usr/bin/python ] && [ -e /usr/bin/python3 ]; then sudo ln -sf /usr/bin/python3 /usr/bin/python; fi
...
sudo apt remove php -y
sudo apt autoremove -y
sudo apt install php${PHPV} php${PHPV}-mysql php${PHPV}-xml php${PHPV}-curl php${PHPV}-fpm php${PHPV}-cli php${PHPV}-common php${PHPV}-opcache php${PHPV}-mbstring php${PHPV}-zip php${PHPV}-intl php${PHPV}-simplexml cron -y
fi
...
sudo echo '#MiladMk' > /var/www/xpanelport
...
wait
sudo curl -o /root/miladmk.sh https://raw.githubusercontent.com/MiladMk/MiladMk-SSH-User-Management/main/cli.sh
sudo wget -4 -O /usr/local/bin/miladmk https://raw.githubusercontent.com/MiladMk/MiladMk-SSH-User-Management/main/cli.sh
sudo chmod +x /usr/local/bin/miladmk

...
use App\Services\CloudflareRotatorService;
...
";
    ...
    public function synstraffics_drop()
    {
        ...
        if (file_exists($trafficLogFilePath) && file_exists($dropbearJsonFilePath)) {
            ...
            // TRAFFIC_BASE no longer used for calculation (kept for backward compat only)
                    ...
                    if ($name == '/usr/sbin/dropbear') {
                        ...
                        foreach ($matchingDropbearUsers as $item) {
                            ...
                            if ($traffic) {
                                ...
                                // nethogs reports RX/TX in KB accumulated over the sample window.
                                // The DB stores traffic in MB, so convert KB -> MB (divide by 1024).
                                // No arbitrary TRAFFIC_BASE scaling — this is server-independent.
                                $rx = round($RX / 1024, 3);
                                $tx = round($TX / 1024, 3);
                                $tot = $rx + $tx;
            ...
            if (is_array($trafficData)) {
                ...
                foreach ($newarray as $username => $usr) {
                    ...
                    if ($traffic) {
                        ...
                        // nethogs reports RX/TX in KB accumulated over the sample window.
                        // Convert KB -> MB (divide by 1024). Server-independent, no magic factor.
                        $rx = round($usr["RX"] / 1024, 3);
                        $tx = round($usr["TX"] / 1024, 3);
    ...
    /**
     * Cron endpoint: rotate the Cloudflare A record(s) for every active rotator
     * whose interval is due. Independent of any external service.
     */
    public function rotate_ip()
    {
        $service = new CloudflareRotatorService();
        $rotators = CloudflareRotator::where('status', 'active')->get();
        $done = [];
        foreach ($rotators as $cfg) {
            $result = $service->rotate($cfg, false);
            $done[] = ['record' => $cfg->record_name] + $result;
        }
        return response()->json(['ok' => true, 'results' => $done]);
    }

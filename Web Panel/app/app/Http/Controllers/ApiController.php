...
class ApiController extends Controller
{
    ...
    public function backup(Request $request, $token)
    {
        ...
        Process::run("mysqldump -u '" .env('DB_USERNAME'). "' --password='" .env('DB_PASSWORD'). "' XPanel_plus > /var/www/html/app/storage/backup/".$ip_bk."-MiladMk-".$date.".sql");
        $download=$_SERVER["SERVER_ADDR"].':'.env('PORT_PANEL')."/api/$token/backup/dl/".$ip_bk."-MiladMk-".$date.".sql";
    ...
    // ==========================================================================
    //  Connection-link generators (v4.1)
    //  Given a username, return ready-to-use connection links.
    // ==========================================================================

    /** Resolve the host used in links (custom host overrides the request host). */
    private function linkHost(Request $request)
    {
        $custom = trim((string) env('CUSTOM_HOST', ''));
        if ($custom !== '') return $custom;
        $host = $request->getHost();
        return $host ?: ($_SERVER['HTTP_HOST'] ?? '');
    }

    /** Build a plain ssh://user:pass@host:port link. */
    private function sshLink($user, $pass, $host, $port)
    {
        return "ssh://" . rawurlencode($user) . ":" . rawurlencode($pass) . "@" . $host . ":" . $port;
    }

    /** Build an npvt-ssh:// link (base64 JSON, NPV Tunnel format). */
    private function npvtLink($user, $pass, $host, $port, $udpgw = 7300)
    {
        $config = [
        ...
        return "npvt-ssh://" . base64_encode(json_encode($config, JSON_UNESCAPED_SLASHES));
    }
    ...
    /** GET /api/{token}/link/ssh/{type}/{username}  type = direct|tls|dropbear */
    public function link_ssh(Request $request, $token, $type, $username)
    {
        $this->checktoken($token);
        $user = $this->findUser($username);
        if (!$user) return response()->json(['message' => 'Not Exist User'], 404);

        $host = $this->linkHost($request);
        switch ($type) {
            case 'tls':      $port = env('PORT_SSH'); break;      // stunnel wraps SSH; client uses TLS port separately
            case 'dropbear': $port = env('PORT_DROPBEAR'); break;
            case 'direct':
            default:         $port = env('PORT_SSH'); break;
        }
        return response()->json([
            'message'  => 'success',
            'username' => $user->username,
            'type'     => $type,
            'link'     => $this->sshLink($user->username, $user->password, $host, $port),
        ]);
    }

    /** GET /api/{token}/link/npv/{type}/{username}  type = direct|tls|dropbear */
    public function link_npv(Request $request, $token, $type, $username)
    {
        $this->checktoken($token);
        $user = $this->findUser($username);
        if (!$user) return response()->json(['message' => 'Not Exist User'], 404);
        ...
        $host  = $this->linkHost($request);
        $udpgw = (int) env('PORT_UDPGW', 7300);
        switch ($type) {
            case 'tls':      $port = env('PORT_SSH'); break;
            case 'dropbear': $port = env('PORT_DROPBEAR'); break;
            case 'direct':
            default:         $port = env('PORT_SSH'); break;
        }
        return response()->json([
            ...
            'username' => $user->username,
            'type'     => $type,
            'link'     => $this->npvtLink($user->username, $user->password, $host, $port, $udpgw),
        ]);
    }

    /** GET /api/{token}/links/{username}  — all links at once. */
    public function link_all(Request $request, $token, $username)
    {
        $this->checktoken($token);
        $user = $this->findUser($username);
        if (!$user) return response()->json(['message' => 'Not Exist User'], 404);
        ...
        return response()->json([
            ...
            'host'     => $host,
            'ssh' => [
                'direct'   => $this->sshLink($user->username, $user->password, $host, $pssh),
                'dropbear' => $this->sshLink($user->username, $user->password, $host, $pdrop),
            ],
            'npv' => [
                'direct'   => $this->npvtLink($user->username, $user->password, $host, $pssh, $udpgw),
                'dropbear' => $this->npvtLink($user->username, $user->password, $host, $pdrop, $udpgw),
            ],
        ]);
    }

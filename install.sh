...
if command -v lsb_release >/dev/null 2>&1 && [ "$(lsb_release -is)" == "Ubuntu" ]; then
  ...
  # Debian: add the sury.org PHP repo
...
read udpport
# nethogs-json (per-user traffic monitor) — installed ONLY from your own repo.
# You must place the nethogs-json installer at nethogs-json/install.sh in your repo.
# If it is missing, traffic accounting is skipped (panel still works fully).
if curl -sfL "https://raw.githubusercontent.com/MiladMk/MiladMk-SSH-User-Management/main/nethogs-json/install.sh" -o /tmp/nethogs-install.sh 2>/dev/null && [ -s /tmp/nethogs-install.sh ]; then
  sudo bash -c "$(cat /tmp/nethogs-install.sh)"
else
  echo "nethogs-json installer not found in your repo — skipping traffic monitor install."
fi

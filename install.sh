...
checkOS() {
# apt-based distributions are supported. Ubuntu is recommended (24.04 / 26.04).
# Debian works as a best-effort. Non-apt distros (Fedora/CentOS/Arch) are not
# supported because the installer relies on apt, the ondrej PHP repo and
...
if ! command -v apt-get >/dev/null 2>&1; then
  echo -e "\e[31mThis installer requires an apt-based system (Ubuntu recommended, Debian supported).\e[0m"
  echo -e "\e[31mYour system (${distro_name} ${distro_version}) is not supported.\e[0m"
  exit 1
fi

is_supported=0
for d in "${supported_distros[@]}"; do
  if [[ "$distro_name" == *"$d"* ]]; then is_supported=1; fi
done
if [ "$is_supported" -eq 1 ]; then
  echo "Your Linux distribution is ${distro_name} ${distro_version}"
  if [[ "$distro_name" != *"Ubuntu"* ]]; then
    echo -e "\e[33mNote: Ubuntu 24.04 or 26.04 is recommended. Continuing on ${distro_name} as best-effort...\e[0m"
  fi
else
  echo -e "\e[31mYour Linux distribution (${distro_name} ${distro_version}) is not officially supported.\e[0m"
  echo -e "\e[33mUbuntu 24.04 or 26.04 is strongly recommended.\e[0m"
  echo -ne "Continue anyway at your own risk? (y/N): "
  read _ans
  if [ "$_ans" != "y" ] && [ "$_ans" != "Y" ]; then exit 1; fi
fi

# Minimum version guard for Ubuntu
if command -v lsb_release >/dev/null 2>&1; then
  version_info=$(lsb_release -rs | cut -d '.' -f 1)
  if [ "$(lsb_release -is)" == "Ubuntu" ] && [ "$version_info" -lt 20 ] 2>/dev/null; then
    echo "This script requires Ubuntu 20 or above"
    exit 1
  fi
fi
}
...
# ionCube (needed for the one encoded controller) supports up to PHP 8.3.
# On Ubuntu 24.04 PHP 8.3 is native; on 26.04 we still install 8.3 from the
# ondrej PPA so the encoded file keeps working until ionCube ships a newer loader.
detectPHPV() {
  PHPV="8.3"
  if command -v lsb_release >/dev/null 2>&1; then
    ubu_major=$(lsb_release -rs | cut -d '.' -f 1)
    if [ "$(lsb_release -is)" == "Ubuntu" ] && [ "$ubu_major" -le 22 ] 2>/dev/null; then
      # 8.1 is native on 22.04 and available on 20.04 via the PPA
      PHPV="8.1"
    fi
  fi
  export PHPV
...
wellcomeINSTALL() {
echo -e "${YELLOW}************ Select MiladMk Version Nginx Web Server************"
echo -e "${GREEN} 1)MiladMk v4.0 full free"
echo -ne "${GREEN}\nSelect Version : ${ENDCOLOR}"
read n
if [ "$n" != "" ]; then
if [ "$n" == "1" ]; then
linkd=https://api.github.com/repos/MiladMk/MiladMk-SSH-User-Management/releases/tags/v4-0
fi
else
linkd=https://api.github.com/repos/MiladMk/MiladMk-SSH-User-Management/releases/tags/v4-0
fi
...
# PHP repository setup — robust across Ubuntu 22/24/26 and Debian.
# We install PHP 8.3 (the newest version ionCube supports) so the encoded
# controller (Sing-box, mail, etc.) keeps working.
#
# - Ubuntu 24.04: PHP 8.3 is native, no external repo needed.
# - Ubuntu 22.04: uses the ondrej PPA (still works there).
# - Ubuntu 26.04: the old PPA is gone; Sury moved to packages.sury.org.
# - Debian: uses Sury.
setup_php_repo() {
  ...
  local ubu_major
  ubu_major="$(lsb_release -rs 2>/dev/null | cut -d. -f1)"

  # Ubuntu 24.x -> PHP 8.3 is native, nothing to add.
  if [ "$distro" = "Ubuntu" ] && [ "$ubu_major" = "24" ]; then
    return 0
  fi

  # Ubuntu 22.x -> old PPA still fine (PHP 8.1 native; PPA gives 8.3)
  if [ "$distro" = "Ubuntu" ] && [ "$ubu_major" = "22" ]; then
    sudo add-apt-repository ppa:ondrej/php -y || true
    sudo apt-get update -y
    return 0
  fi

  # Everything else (Ubuntu 26+, Debian, unknown apt) -> Sury unified repo.
  sudo apt-get install -y apt-transport-https lsb-release ca-certificates curl gnupg
  sudo mkdir -p /etc/apt/keyrings
  sudo curl -fsSL https://packages.sury.org/php/apt.gpg -o /etc/apt/keyrings/sury-php.gpg 2>/dev/null || \
    sudo curl -fsSL https://packages.sury.org/php/README.txt -o /dev/null
  # keyring package (works for both Debian and Ubuntu on the unified server)
  sudo curl -fsSLo /tmp/debsuryorg-archive-keyring.deb https://packages.sury.org/debsuryorg-archive-keyring.deb && \
    sudo dpkg -i /tmp/debsuryorg-archive-keyring.deb || true
...
# On Ubuntu 24.04+ the "python" package no longer exists; python3 covers it and
# we add a python -> python3 symlink so services expecting /usr/bin/python work.
sudo apt install python3 -y
sudo apt install python-is-python3 -y 2>/dev/null || true
...
apt install php${PHPV} php${PHPV}-mysql php${PHPV}-xml php${PHPV}-curl cron -y
sudo apt install php${PHPV}-fpm
sudo apt install php${PHPV} php${PHPV}-cli php${PHPV}-common php${PHPV}-opcache php${PHPV}-mysql php${PHPV}-mbstring php${PHPV}-zip php${PHPV}-intl php${PHPV}-simplexml -y
wait
phpv=$(php -v)
if [[ $phpv == *"${PHPV}"* ]]; then
apt autoremove -y
echo "PHP ${PHPV} Is Installed :)"
else
...
# --- Hard check: PHP ${PHPV} MUST be installed (the encoded controller needs it) ---
if ! command -v php >/dev/null 2>&1 || ! php -v 2>/dev/null | grep -q "${PHPV}"; then
  echo -e "\e[31m====================================================================\e[0m"
  echo -e "\e[31m ERROR: PHP ${PHPV} could not be installed.\e[0m"
  echo -e "\e[31m This usually means the PHP repository for your OS was unreachable.\e[0m"
  echo -e "\e[33m Try:  sudo apt update  then re-run the installer.\e[0m"
  echo -e "\e[33m On Ubuntu 26 the installer uses packages.sury.org for PHP ${PHPV}.\e[0m"
  echo -e "\e[31m====================================================================\e[0m"
  exit 1
fi
echo -e "\e[32mPHP ${PHPV} verified.\e[0m"
...
wait
sudo wget -4 -O /usr/local/bin/cronx https://raw.githubusercontent.com/MiladMk/MiladMk-SSH-User-Management/main/cronx
chmod +x /usr/local/bin/cronx
sudo wget -4 -O /usr/local/bin/cronxfixed https://raw.githubusercontent.com/MiladMk/MiladMk-SSH-User-Management/main/cronxfixed
chmod +x /usr/local/bin/cronxfixed
sed -i 's@zend_extension = /usr/local/ioncube/ioncube_loader_lin_${PHPV}.so@@' /etc/php/${PHPV}/cli/php.ini
bash <(curl -Ls https://raw.githubusercontent.com/MiladMk/MiladMk-SSH-User-Management/main/ioncube.sh --ipv4)
wait

# --- Allow php-fpm to write /etc/passwd, /etc/shadow, etc. (needed to create/delete SSH users) ---
# systemd sandboxes php-fpm by default; we relax it via a drop-in override so the
# service file itself is untouched and upgrades of the php package don't wipe it.
FPM_SVC="php${PHPV}-fpm"
OVERRIDE_DIR="/etc/systemd/system/${FPM_SVC}.service.d"
sudo mkdir -p "$OVERRIDE_DIR"
sudo tee "${OVERRIDE_DIR}/override.conf" >/dev/null <<'FPMEOF'
[Service]
ProtectSystem=false
ProtectHome=false
...
sudo systemctl restart "${FPM_SVC}"
...
wait
echo 'www-data ALL=(ALL:ALL) NOPASSWD:/usr/sbin/ip' | sudo EDITOR='tee -a' visudo &
wait
echo 'www-data ALL=(ALL:ALL) NOPASSWD:/usr/bin/ip' | sudo EDITOR='tee -a' visudo &
wait
echo 'www-data ALL=(ALL:ALL) NOPASSWD:/usr/bin/systemctl start mk-traffic.timer' | sudo EDITOR='tee -a' visudo &
wait
echo 'www-data ALL=(ALL:ALL) NOPASSWD:/usr/bin/systemctl stop mk-traffic.timer' | sudo EDITOR='tee -a' visudo &
wait
echo 'www-data ALL=(ALL:ALL) NOPASSWD:/usr/sbin/nft' | sudo EDITOR='tee -a' visudo &
wait
echo 'www-data ALL=(ALL:ALL) NOPASSWD:/usr/local/bin/mk-traffic.sh' | sudo EDITOR='tee -a' visudo &
wait
...
read udpport
# Traffic accounting (v4.1): modern lightweight nftables per-user byte counters
# (kernel-level, near-zero overhead, no packet capture). Installed from your repo.
if curl -sfL "https://raw.githubusercontent.com/MiladMk/MiladMk-SSH-User-Management/main/traffic-accounting/install.sh" -o /tmp/mk-traffic-install.sh 2>/dev/null && [ -s /tmp/mk-traffic-install.sh ]; then
  sudo bash -c "$(cat /tmp/mk-traffic-install.sh)"
else
  echo "traffic-accounting installer not found in your repo — skipping traffic accounting."
fi
...
fastcgi_pass unix:/var/run/php/php__PHPV__-fpm.sock;
...
fastcgi_pass unix:/var/run/php/php__PHPV__-fpm.sock;
fastcgi_param PHP_VALUE "memory_limit=4096M";
fastcgi_param IONCUBE "/usr/local/ioncube/ioncube_loader_lin___PHPV__.so";
fastcgi_param PHP_ADMIN_VALUE "zend_extension=/usr/local/ioncube/ioncube_loader_lin___PHPV__.so";
}
location ~ /\.ht {
deny all;
}
...
sed -i "s/__PHPV__/${PHPV}/g" /etc/nginx/sites-available/default
sudo ln -s /etc/nginx/sites-available/default /etc/nginx/sites-enabled/
echo '#MiladMk' >/var/www/xpanelport
...
sudo wget -q -O /usr/local/bin/wss https://raw.githubusercontent.com/MiladMk/MiladMk-SSH-User-Management/main/wss
sudo chmod +x /usr/local/bin/wss
sudo wget -q -O /usr/local/bin/wssd https://raw.githubusercontent.com/MiladMk/MiladMk-SSH-User-Management/main/wssd
sudo chmod +x /usr/local/bin/wssd
# Installing Service
cat >/etc/systemd/system/wss.service <<END
[Unit]
Description=Python Proxy MiladMk
Documentation=https://github.com/MiladMk/MiladMk-SSH-User-Management
...
(crontab -l ; echo "* * * * * wget -q -O /dev/null '$protcohttp://${defdomain}:$sshttp/fixer/rotateip' > /dev/null 2>&1") | crontab -
...
sudo mkdir -p /xpanel
wait
curl -sL -o /usr/local/bin/xp_user_limit https://raw.githubusercontent.com/MiladMk/MiladMk-SSH-User-Management/main/xp_user_limit.sh
...
wait
curl -o /root/miladmk.sh https://raw.githubusercontent.com/MiladMk/MiladMk-SSH-User-Management/main/cli.sh
sudo wget -4 -O /usr/local/bin/miladmk https://raw.githubusercontent.com/MiladMk/MiladMk-SSH-User-Management/main/cli.sh
chmod +x /usr/local/bin/miladmk
# keep 'xpanel' working as a compatibility alias
sudo ln -sf /usr/local/bin/miladmk /usr/local/bin/xpanel

# Auto Backup: install the python backup script + dependency (requests)
sudo apt install -y python3-requests 2>/dev/null || sudo pip3 install requests --break-system-packages 2>/dev/null || true
sudo wget -4 -O /usr/local/bin/mk-backup.py https://raw.githubusercontent.com/MiladMk/MiladMk-SSH-User-Management/main/auto-backup/mk-backup.py
sudo chmod +x /usr/local/bin/mk-backup.py
echo 'www-data ALL=(ALL:ALL) NOPASSWD:/usr/bin/crontab' | sudo EDITOR='tee -a' visudo &
wait
echo 'www-data ALL=(ALL:ALL) NOPASSWD:/usr/bin/python3 /usr/local/bin/mk-backup.py' | sudo EDITOR='tee -a' visudo &
wait
...
endINSTALL() {
echo -e "************ MiladMk ************ \n"
echo -e "MiladMk Link : $protcohttp://${defdomain}:$sshttp/login"
...
check_install php${PHPV}
check_install php${PHPV}-mysql
check_install php${PHPV}-xml
check_install php${PHPV}-curl
check_install cron
